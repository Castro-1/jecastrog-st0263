...
class UploadService(service_pb2_grpc.UploadServiceServicer):
    def Upload(self, request, context):
        ...
        if name in self.resources:
            return service_pb2.UploadResponse(success=False, name="")
...
def serve_resources(peer_address: str):
    global resources
    ...
    server.add_insecure_port(peer_address)
    server.start()
...
def run():
    ...
    grpc_thread = threading.Thread(target=serve_resources, args=(peer_address,))
